...
# MAGIC %md
# MAGIC # Formula 1 Data Platform — Enriched: sprint_results
# MAGIC
# MAGIC **Notebook**: `02_enriched/03_enrich_sprint_results.py`
# MAGIC **Sprint**: Issue #5 — Enriched Layer Pipeline
# MAGIC **Layer**: Enriched
# MAGIC **Purpose**: Cleanse, validate, and merge `sprint_results` from Raw into Enriched. Applies DQ rules SR-01 through SR-13. Identical logic to race_results. Timedelta UDF handles Ocon's missing-microseconds format.
...
df = spark.table(f"{CATALOG}.{RAW_SCHEMA}.sprint_results")
rows_read = df.count()
...
if ROUND_NUMBER is None or ROUND_NUMBER < 0 or ROUND_NUMBER > 24:
    ...
    write_quarantine(all_quarantine, CATALOG, "sprint_results")
...
# SR-01: DriverId not null (NULL_PRIMARY_KEY)
...
# SR-02: DriverNumber not null (NULL_DRIVER_NUMBER)
...
# SR-03: TeamId not null (NULL_TEAM_ID)
...
# SR-09: ClassifiedPosition format (INVALID_CLASSIFIED_POSITION)
...
# SR-10: Time format when not null (INVALID_LAP_TIME_FORMAT)
...
# SR-13: TeamColor format (INVALID_TEAM_COLOR_FORMAT)
...
# SR-05: Position range (POSITION_OUT_OF_RANGE)
pos_oor = df.filter(
...
# SR-06: GridPosition range (GRID_POSITION_OUT_OF_RANGE)
grid_oor = df.filter(
...
# SR-07: Points non-negative (NEGATIVE_POINTS)
neg_pts = df.filter(
...
# SR-08: Laps non-negative (NEGATIVE_LAPS)
neg_laps = df.filter(
...
# SR-04: Duplicate primary key (DUPLICATE_PRIMARY_KEY)
...
if non_empty:
    ...
    write_quarantine(quarantine_df, CATALOG, "sprint_results")
else:
    rows_quarantined = 0
...
merge_into_enriched(spark, df, CATALOG, ENRICHED_SCHEMA, "sprint_results", pk_cols)
